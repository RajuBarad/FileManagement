...
// Regex finding `renameSelectedItem() {` with any preceding whitespace
...
if (!content.includes("openMoveDialog()") && regex.test(content)) {
    ...
    console.log("openMoveDialog method injected using regex.");
} else {
    console.log("Method already exists or renameSelectedItem not found using regex.");
}

fs.writeFileSync(filepath, content, 'utf-8');
console.log("Injection Fix Executed.");
